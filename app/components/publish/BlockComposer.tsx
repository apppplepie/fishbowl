...
import { Form, Dropdown } from '@/app/components/ui/compat';
import type { MenuProps } from '@/app/components/ui/compat';
import { Button, Card, Space, Divider, Tag, message, Input } from '@/app/components/ui';
...
/**
 * 块编辑器发布页的共享实现。
 * 「发布文章」与「发布章节」的唯一区别是分类来源、文案与跳转目标，全部通过 props 传入。
 */
export interface BlockComposerProps {
  ...
  exitPath: string;
  /**
   * 固定分类：传入则不显示分类选择器，直接发布到该分类（章节模式）；
   * 传 undefined 表示由用户在表单里选择（文章模式）。
   */
  fixedCategory?: {
    id: string | null;
    name?: string;
  ...
  /** 移动端信息卡片内边距 */
  mobileInfoPadding?: string;
  /** 移动端编辑器卡片内边距 */
  mobileEditorPadding?: string;
}
...
export default function BlockComposer({
  ...
  const usesFixedCategory = fixedCategory !== undefined;
  ...
  const applyDraft = React.useCallback((draft: Article) => {
    form.setFieldsValue({
      title: draft.title,
      tags: draft.tags,
      ...(usesFixedCategory ? {} : { category_id: draft.category_id ?? undefined }),
    });
    setBlocks(draft.blocks || []);
  }, [form, usesFixedCategory]);
  ...
  React.useEffect(() => {
    ...
    if (!draftStr) return;
    try {
      applyDraft(JSON.parse(draftStr) as Article);
      if (process.env.NODE_ENV === 'development') {
        console.log(`✅ 自动加载了${noun}草稿`);
      }
    } catch (error) {
  ...
  const buildDraft = React.useCallback((): Article => {
    ...
    return {
      ...
      tags: values.tags || [],
      category_id: (usesFixedCategory ? fixedCategory?.id : values.category_id) || null,
      blocks: blocksRef.current,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
  }, [form, user?.username, usesFixedCategory, fixedCategory?.id]);
  ...
  const onFinish = async (values: any) => {
    ...
    if (usesFixedCategory && !fixedCategory?.id) {
      message.error(`缺少目录参数，无法发布${noun}`);
      setIsSubmitting(false);
      return;
    }
    ...
    // 分类与排序：固定分类模式直接用传入值，否则按表单选择计算
    let categoryId: string;
    let orderInCategory: number;
    if (usesFixedCategory) {
      categoryId = fixedCategory!.id as string;
      orderInCategory = fixedCategory!.orderIndex;
    } else {
      categoryId = values.category_id || 'cat_uncategorized';
      orderInCategory = 0;
      try {
    ...
    try {
      ...
      console.error(`❌ 发布${noun}异常:`, error);
      ...
      <div style={getContentAreaWrapperStyle()}>
        ...
        >
          <Card
            ...
            bodyStyle={{ padding: isMobile ? mobileInfoPadding : '24px' }}
          >
            {/* 移动端隐藏标题和草稿按钮，使用浮动按钮代替 */}
            {!isMobile && (
              <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                {headerTitle && <h3 style={{ margin: 0, fontSize: '18px' }}>{headerTitle}</h3>}
                <Space size="middle">
            ...
            <Form.Item
              label={isMobile ? null : `${noun}标题`}
              name="title"
              rules={[{ required: true, message: `请输入${noun}标题` }]}
            ...
            <Form.Item
              ...
              name="tags"
              tooltip={isMobile ? null : `添加标签可以帮助读者更好地找到你的${noun}`}
              style={{ marginBottom: isMobile ? '12px' : '24px' }}
            >
            ...
            {usesFixedCategory ? (
              fixedCategory?.id && (
                <div style={{
                  ...
                  <strong>📂 发布到目录：</strong>{fixedCategory.name || fixedCategory.id}
                  {!isMobile && <br />}
                  {isMobile && ' '}
                  <strong>🔢 序号：</strong>{fixedCategory.orderIndex}
                </div>
              )
            ) : (
              <Form.Item
                label={isMobile ? null : `${noun}目录`}
                name="category_id"
                tooltip={isMobile ? null : `选择${noun}所属分类目录，方便管理和查找`}
                style={{ marginBottom: isMobile ? '0' : '24px' }}
              >
                <CategoryTreeSelect placeholder={isMobile ? "选择目录（可选）" : `选择${noun}所属目录（可选）`} />
              </Form.Item>
            )}
          </Card>
          ...
          <Card
            ...
            bodyStyle={{ padding: isMobile ? mobileEditorPadding : '24px' }}
          >
            <div style={{
            ...
            {isPreviewMode ? (
              ...
              <div style={{
                ...
                <h1 style={{
                  ...
                  {form.getFieldValue('title') || `未命名${noun}`}
                </h1>
